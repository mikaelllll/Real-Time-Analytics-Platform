...
def test_parse_ignores_invalid_coordinates() -> None:
    row = [
        "abc123",
        "TEST1",
        "Brazil",
    ...
    assert OpenSkyCollector._parse({"time": 1_700_000_000, "states": [row]}) == []
...
def test_snapshot_reports_states_without_coordinates_without_mapping_them() -> None:
    row = [
        "abc123",
        "TEST1",
        "Brazil",
    ...
    snapshot = AircraftEventProcessor.build_snapshot([state], latency=100, interval=30)

    assert snapshot.aircraft_tracked == 1
    assert snapshot.aircraft_with_position == 0
